...
pdp <- PeakDensityParam(sampleGroups = pData(processedData)$group, bw = 15,
                        minFraction = 0.1, binSize = 0.05)

processedData <- groupChromPeaks(processedData, pdp)


#group peaks
pdp <- PeakDensityParam(sampleGroups = pData(processedData)$group, bw=15, binSize=0.05, minSamples=1, minFraction = 0.01)
...
mse_DF_single$precursorMZ <- as.numeric(sapply(strsplit(as.character(mse_DF_single$precursorMZ_Intensity), " "), `[`, 1))
mse_DF_single$precursorMZ_TIC <- as.numeric(sapply(strsplit(as.character(mse_DF_single$precursorMZ_Intensity), " "), `[`, 2))
...
for(k in feature_order) {
  if (k== feature_order[1]) {text_to_write = c(text_to_write,paste("COM=Experimentexported on", Sys.time(),"\n",sep = "\t"))}
  feature_data <- xset_Matched[which(xset_Matched$Row.names == k),c("mz","rt","feature_id","pcgroup","msms_scan","file")]
  if(feature_data$msms_scan != "no_match"){
    ind_spec <- mse_spec_DF[which(feature_data$file == mse_spec_DF$file & as.numeric(feature_data$msms_scan) == mse_spec_DF$scan),]
    ind_spec_peaks <- c()
    for(n in 1:nrow(ind_spec)){ind_spec_peaks <- c(ind_spec_peaks,paste(ind_spec[n,3],ind_spec[n,4],sep="\t"))}
...
write(text_to_write, file=here("results","./mgf_test.mgf"))
...
names(xset_Matched)
"Row.names"	mzmed	mzmin	mzmax	rtmed	rtmin	rtmax	npeaks	sample	10712_P4_RH3_01_521.mzXML	10715_P4_RA4_01_415.mzXML	10765_P4_RE9_01_482.mzXML	11035_P4_RB4_01_431.mzXML	13917_P4_RA10_01_423.mzXML	14153_P4_RH8_01_526.mzXML	14880_P3_RA3_01_290.mzXML	18404_P3_RF6_01_370.mzXML	27373_P2_RD5_01_204.mzXML	29342_P5_RF3_01_620.mzXML	31878_P1_RH3_01_131.mzXML	10232_P4_RE4_01_476.mzXML	10546_P4_RG11_01_515.mzXML	11111_P4_RB1_01_428.mzXML	11163_P4_RB11_01_439.mzXML	12262_P1_RE5_01_94.mzXML	13541_P4_RG3_01_506.mzXML	14563_P1_RE10_01_99.mzXML	15581_P2_RD4_01_203.mzXML	15623_P2_RC3_01_187.mzXML	16500_P2_RD8_01_209.mzXML	16501_P2_RD9_01_210.mzXML
write.table(xset_Matched, here("results","feauter_abundance_table_all_features.txt"), sep = "\t", quote = FALSE, row.names = FALSE)
