...
UPLC_Results_Master <- read.csv("./UPLC_RESULTS_AMAZON_PROJECT_JUNE2022_V7.csv")

#First we  subset to a specifc project....
project <- "DUROIA"

UPLC_Results <- UPLC_Results_Master[UPLC_Results_Master[,project] == "YES" | UPLC_Results_Master[,"All_Included_RTIs"] == "YES",]
dim(UPLC_Results)


if (.Platform$OS.type == "unix") {
  register(SerialParam(), default=TRUE)
  UPLC_Results$Converted_Check <- gsub("\\","/",UPLC_Results$Converted_Check,fixed=T)
  UPLC_Results$MSE_Check <- gsub("\\","/",UPLC_Results$MSE_Check,fixed=T)
}
...
pd[duplicated(pd$sample),]
#Read all of the data as an XCMS project.
#this slow but can't be spead up with par processing. Best on fast hard drives.
rawData <- readMSData(pd$path, pdata = new("NAnnotatedDataFrame", pd), centroided. = TRUE,mode= "onDisk", verbose=T, msLevel. = 1L)

#Peak chromatographic peaks
#slowest step, this in theory can be sped up with par processing but might break
cwp <- CentWaveParam(peakwidth = c(2, 10), ppm=15, integrate = 2, snthresh=0,noise = 1500, prefilter = c(5,2000))

processedData <- findChromPeaks(rawData, param = cwp)

mpp <- MergeNeighboringPeaksParam(expandRt = 4)
processedData <- refineChromPeaks(processedData, param = mpp)


#group peaks
pdp <- PeakDensityParam(sampleGroups = pData(processedData)$group, bw=7, binSize=0.05, minSamples=1, minFraction = 0.01)
...
pgp <- PeakGroupsParam(minFraction = 0.0001, span = 0.5,subset=which(pData(processedData)$group == "Standard"),subsetAdjust = "previous")

processedData <- adjustRtime(processedData, param = pgp) 


#regroup peaks
pdp <- PeakDensityParam(sampleGroups = pData(processedData)$group, bw=7, binSize=0.25, minSamples=1, minFraction = 0.001)
processedData <- groupChromPeaks(processedData, param = pdp)
...
#Get Features Definitions and output for GNPS
...
abund_df <- dataTable[,seq(14,13+nsamples,1)]
abund_df[is.na(abund_df)] <- 0
dataTable_for_filtering <- dataTable[,c("Row.names","mzmed","rtmed")]

#remove peaks found in the blank
dataTable_for_filtering$Blank_max_value <- as.numeric(apply(abund_df[,which(pData(processedData)$group == "Blank")], 1, max))
dataTable_for_filtering$max_value <- as.numeric(apply(abund_df, 1, max))
dataTable_for_filtering$remove_found_in_blank <- dataTable_for_filtering$Blank_max_value > 1000

#remove peaks found in the first 30 seconds or after 11 minutes
dataTable_for_filtering$remove_retention_time_window <- !c(dataTable_for_filtering$rtmed > 25 & dataTable_for_filtering$rtmed < 660)

#remove peaks with a max value less than 5000
dataTable_for_filtering$remove_max_value_5000 <- dataTable_for_filtering$max_value < 5000

table(dataTable_for_filtering$remove_found_in_blank == F & dataTable_for_filtering$remove_retention_time_window == F & dataTable_for_filtering$remove_max_value_5000 == F)

dataTable_keep <- dataTable_for_filtering[dataTable_for_filtering$remove_found_in_blank == F & dataTable_for_filtering$remove_retention_time_window == F & dataTable_for_filtering$remove_max_value_5000 == F,]

#filter datatable for export
dataTable <- dataTable[dataTable$Row.names %in% dataTable_keep$Row.names,]
...
MSE_deconvoluted <- paste0("./Duroia/data/MSE/",list.files(path = "./Duroia/data/MSE/",pattern = ".mgf"))
...
peak_id <- gsub("CP","",row.names(sample_chrom_peaks))
peak_id <- gsub("M","",peak_id)
sample_chrom_peaks$peakid <- as.numeric(peak_id)
