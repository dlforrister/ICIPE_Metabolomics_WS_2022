...
head(mse_DF)
x=features_spec$row_number[which.min(abs(features_spec$mz - 301.0354))]
pot_feature <- features_spec[features_spec$mz >= mzmin & features_spec$mz <= mzmax & features_spec$rt >= rtmin & features_spec$rt <= rtmax,]
ms_ms_match_results_df[ms_ms_match_results_df$mz >= mzmin & ms_ms_match_results_df$mz <= mzmax,]
...
match_features <- function(x){
  ...
  if(nrow(pot_feature) > 1) {
    ...
    closest_feature <- pot_feature[which.min(abs(pot_feature$precursorMZ - mz)*10000+abs(pot_feature$retentionTime - rt)),]
...
length(unique(ms_ms_match_results_df$feature_id[ms_ms_match_results_df$msms_scan !="no_match"]))
length(unique(ms_ms_match_results_df$feature_id))
#merge onto xset8
