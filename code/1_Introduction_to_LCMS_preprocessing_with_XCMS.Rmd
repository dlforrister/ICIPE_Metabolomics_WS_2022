...
We can adjust the signal to noise threshold parameter and see if we can pick it up.

If we adjust snthreth we see that the only way to pick up the whole peak is to set it to zero. This is fine, we just have to rely on other parameters to filter out noise.

We also have to adjust the width of the peak because this is a wider peak

```{r}

cwp <- CentWaveParam(peakwidth = c(4, 10),ppm=15, integrate = 2, snthresh = 0)
...
We can change the `noise` and the `prefilter` parameters to exclude baseline peaks.
...
Tryptophan has a TIC fo 20,000 in a single scan

Even the smaller peaks come out at around 2500 in a single scan, and are approaching the baseline much below that.

To differentiate noise from a true peak, we can use the prefilter to exclude peaks that don't have a certain threshold in n number of consecutive scans.

prefilter = c(5,5000)

will exclude any peaks that don't have a TIC of 5,000 in 5 consecative scans.

Lets apply this and test if we pick up Tryptophan

```{r}

cwp <- CentWaveParam(peakwidth = c(3, 10), integrate = 2,snthresh = 0,noise = 1500, prefilter = c(5,2000))
...
Here we identify everything but it split the peak into 2 chunks. In order to not split them we need a value of 4 and that will not allow us to pick of trypt and some of the other Peaks.

\
Alternatively we can use the function MergeNeighboringPeaks

```{r}
mpp <- MergeNeighboringPeaksParam(expandRt = 2)
single_peak_chr <- refineChromPeaks(single_peak_chr, param = mpp)
plot(single_peak_chr)
```
...
Let's check to make sure that the MergeNeighboring Peaks Function doesn't combine these two distinct peaks...

```{r}
mpp <- MergeNeighboringPeaksParam(expandRt = 4)
single_peak_chr_merged <- refineChromPeaks(single_peak_chr, param = mpp)
plot(single_peak_chr_merged)

chromPeaks(single_peak_chr_merged)
```

It did not combine the two major peaks, and but it did combine one of the shoulder peaks with it's parent. Overall this is good.
...
|  Camphorsulfonic acid   |     230 to 250     | 231.0697  |       4        |        13         |    2     |  This is a very wide peak!  |
...
bpc_raw <- chromatogram(processedData, aggregationFun = "max",rt = c(0, 700))
...
rutin_chr <- chromatogram(processedData, rt = c(rt_min, rt_max),
                        mz = c(mz_min, mz_max),
                        aggregationFun = "max")
...
epi_chr <- chromatogram(processedData, rt = c(rt_min, rt_max),
                        mz = c(mz_min, mz_max),
                        aggregationFun = "max")
...
cam_chr <- chromatogram(processedData, rt = c(rt_min, rt_max),
                        mz = c(mz_min, mz_max),
                        aggregationFun = "max")
...
Dig_chr <- chromatogram(processedData, rt = c(rt_min, rt_max),
                        mz = c(mz_min, mz_max),
                        aggregationFun = "max")
...
pgp <- PeakGroupsParam(minFraction = 0.1, span = 0.6)
#op <- ObiwarpParam(response = 100)
...
plot(chromatogram(processedData, aggregationFun = "max", include = "none",rt = c(0, 700)))
...
We can start with the default parameters and see how they work

```{r}
#' Get default parameters for the grouping
pdp <- PeakDensityParam(sampleGroups = pData(processedData)$group)
...
Now we look at the denisty plot of each one to asses grouping paramaters

Run each set of 3 lines individually and then look at the two outputs.

```{r}

#' Dry-run correspondence and show the results.
plotChromPeakDensity(bpc_raff, param = pdp)
bpc_raff <- groupChromPeaks(bpc_raff, param = pdp) 
featureDefinitions(bpc_raff)[1:6]

#defult parameters work well - single peak, accurate mass

plotChromPeakDensity(bpc_trypt, param = pdp)
bpc_trypt <- groupChromPeaks(bpc_trypt, param = pdp) 
featureDefinitions(bpc_trypt)[1:6]


#defult parameters work well - seperates two peaks, accurate mass

plotChromPeakDensity(bpc_camphor, param = pdp)
bpc_camphor <- groupChromPeaks(bpc_camphor, param = pdp) 
featureDefinitions(bpc_camphor)[1:6]


#defult parameters work well - single peak, accurate mass

plotChromPeakDensity(bpc_epic, param = pdp)
bpc_epic <- groupChromPeaks(bpc_epic, param = pdp) 
featureDefinitions(bpc_epic)[1:6]


#defult parameters work well - three peaks are seperating well, accurate mass

plotChromPeakDensity(bpc_rutin, param = pdp)
bpc_rutin <- groupChromPeaks(bpc_rutin, param = pdp) 
featureDefinitions(bpc_rutin)[1:6]


#defult parameters work well - single peak, accurate mass

plotChromPeakDensity(bpc_morin, param = pdp)
bpc_morin <- groupChromPeaks(bpc_morin, param = pdp) 
featureDefinitions(bpc_morin)[1:6]


#defult parameters work ok - it's grouping in some small baseline peaks together, but they are so low abundance probably doesn't matter, accurate mass

plotChromPeakDensity(bpc_digoxin, param = pdp)
bpc_digoxin <- groupChromPeaks(bpc_digoxin, param = pdp) 
featureDefinitions(bpc_digoxin)[1:6]


#defult parameters work well - two peak, accurate mass

plotChromPeakDensity(bpc_glyc, param = pdp)
bpc_glyc <- groupChromPeaks(bpc_glyc, param = pdp) 
featureDefinitions(bpc_glyc)[1:6]


#defult parameters does not work - it is grouping together two seperate peaks

# we can adjust BW to fix this.

```

we can adjust bw from 25 to smaller values. This will separate masses that are closer in time.

```{r}
#we have to go all the way down to BW = 4 before these two peaks will separate
...
Do these new params still work for the rest of peaks?
...
#narrow parameters work well - single peak, accurate mass
...
#narrow parameters work well - seperates two peaks, accurate mass

plotChromPeakDensity(bpc_camphor, param = pdp)
bpc_camphor <- groupChromPeaks(bpc_camphor, param = pdp) 
featureDefinitions(bpc_camphor)[1:6]

#default paramateres over split this large peak- would need a BW of 10 to NOT split it.
#unfortuneatly this is a trade off. I think it's better to over split a peak than to groupt together two clearly destinct peaks. At least when a peak is over split, the main feature/peak will be included in all samples and the other over split features are low abundance and won't contribute much to the analysis.
...
#narrow parameters work well - three peaks are seperating well, accurate mass
...
#narrow parameters work ok - it's grouping in some small baseline peaks together, but they are so low abundance probably doesn't matter, accurate mass
...
#narrow parameters work well - two peak, accurate mass
...
#narrow parameters work well - it splits the  two seperate peaks

#it's a trade off with comnforsulfonic acid, because a BW of 10 is too broad to split these peaks.

```

Next we group all features across the data set using these newly defined parameters.
...
featureDefinitions(processedData)
```
...
fmat <- featureValues(processedData, value = "into", method = "maxint")
head(fmat)
```
...
sum(is.na(featureValues(processedData)))
```

Last Step:

CAMERA to group things into Psuedo Spectra and add isotopes and adducts

First we convert the "XCMSnExp" object into an xset object so it can work with CAMERA
...
Next we set the polararity and we group peaks:

```{r}
xsa <- xsAnnotate(xset, polarity = "negative")
xsaF <- groupFWHM(xsa, sigma = 6, perfwhm = 0.8)

peakList <- getPeaklist(xsaF)
peakList <- peakList[order(as.numeric(peakList$pcgroup),-peakList$X7),]

View(peakList)
```

We can extract specific compounds to look at and then examine how coeluting features get grouped.
...
peakList_subset <- peakList[peakList$mz >= mz_min & peakList$mz <= mz_max & peakList$rt >= rt_min & peakList$rt <= rt_max,]
...
pcgroup_subset <- peakList_subset$pcgroup

#this identified pcgroup 1

# now what other mz value are co-eluting with the main feature around 609?

peakList[peakList$pcgroup == pcgroup_subset,]
```

```{r}
xsaC <- groupCorr(xsaF, cor_eic_th = 0.25, pval = 0.05, graphMethod = "hcs",
                  calcCiS = TRUE, calcCaS = FALSE, calcIso = FALSE)
peakList_cor <- getPeaklist(xsaC)

peakList_merged <- merge(peakList,peakList_cor[,"pcgroup"],by=0)
peakList_merged <- peakList_merged[order(as.numeric(peakList_merged$pcgroup),-peakList_merged$X7),]


write.csv(peakList_merged,"~/Downloads/peaklist_merged.csv")
